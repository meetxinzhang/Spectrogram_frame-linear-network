...
def model(x, w_fc1, b_fc1, keep_prob_conv, isx):
    h_conv1 = tf.layers.conv3d(inputs=x, filters=8, kernel_size=[3, 3, 3], padding='SAME', use_bias=False,
                               activation=None)
    ...
    h_conv2 = tf.layers.conv3d(inputs=h_conv1_drop, filters=32, kernel_size=[3, 3, 3], padding='SAME', use_bias=False,
                               activation=None)
    ...
    h_conv3 = tf.layers.conv3d(inputs=h_conv2_drop, filters=16, kernel_size=[3, 3, 3], padding='SAME', use_bias=False,
                               activation=None)
...
if __name__ == "__main__":
    ...
    if training:
        ...
        for i in range(150):
            ...
            test_auc, test_loss = sess.run([auc, cost_softmax],
                                           feed_dict={X: teX, Y: teY, p_keep_conv: 1.0, isTrain: False})
    ...
    if testing:
        ...
        with open(directory + '/eval_final_' + str(seed) + '.csv', 'w') as myfile:
