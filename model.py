...
class The3dcnn_lstm_Model(tf.keras.Model):
    ...
    def __init__(self, rnn_units, num_class):
        ...
        # 3DCNN
        self.conv3d1 = tf.keras.layers.Conv3D(filters=8, kernel_size=[3, 78, 3], strides=[1, 1, 1], use_bias=True,
        ...
        self.conv3d2 = tf.keras.layers.Conv3D(filters=16, kernel_size=[3, 78, 3], strides=[1, 1, 1], use_bias=True,
                                              activation=tf.nn.leaky_relu, padding='same',
                                              kernel_initializer=tf.keras.initializers.he_normal(),
                                              bias_initializer=tf.zeros_initializer(),
                                              data_format='channels_first')
        ...
        self.conv3d3 = tf.keras.layers.Conv3D(filters=8, kernel_size=[3, 78, 3], strides=[1, 1, 1], use_bias=True,
                                              activation=tf.nn.leaky_relu, padding='same',
                                              kernel_initializer=tf.keras.initializers.he_normal(),
                                              bias_initializer=tf.zeros_initializer(),
                                              data_format='channels_first')
    ...
    def call(self, inputs, drop_rate=0.3, **kwargs):
        ...
        pool3 = self.pooling3(conv3)  # (?, 8, 2, 10, 5)
        ...
        ##################################################################
        x_rnns = tf.unstack(x_rnn, axis=1)  # 展开通道维度  8*[?, 10, 10]
        x_rnn = tf.concat(x_rnns, axis=2)  # 合并到列维度  [?, 80, 10]
