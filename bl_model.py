...
class XNN(tf.keras.Model):
    ...
    def __init__(self, num_class, rnn_units):
        ...
        self.conv4 = tf.keras.layers.Conv2D(
            filters=1, kernel_size=[3, 3], strides=[1, 1], use_bias=True, activation=tf.nn.relu, padding='same',
    ...
    def call_cnn(self, input):
        ...
        score = self.conv4(pool2)  # (1, 10, 25, 1)

        return pool3.numpy(), score.numpy()
        ...
        for img in inputs:  # (80, width, 1)
            ...
            for (start, end) in self.windows(shape[1], window_size=window_size):
                ...
                if np.shape(signal)[1] == window_size:
                    ...
                    cnn_out, score = self.call_cnn(signal)  # 调用cnn (1, 10, 25, 8), (1, 10, 25, 1)

                    cnn_out = np.squeeze(cnn_out, axis=0)  # 删除batch_size维度 (10, 25, 8)
                    score = np.squeeze(score, axis=None)  # 删除batch_size维度 (10, 25)
                    he = sum(sum(score))
                    if he >= max_score:
                        max_score = he
...
dd = bl_data.batch_generator(file_dir='D:/GitHub/ProjectX/yield/images', num_class=num_class)
